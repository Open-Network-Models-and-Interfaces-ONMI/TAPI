...
class Context_VnwserviceUuidNameValuenameImpl:
    ...
    def put(cls, uuid, valueName, nameandvalue):
        ...
        if uuid in Context._vnwService:
            be.Context._vnwService[uuid].name[valueName] = nameandvalue
        else:
    ...
    def delete(cls, uuid, valueName):
        ...
        if uuid in be.Context._vnwService:
            if valueName in be.Context._vnwService[uuid].name:
                del be.Context._vnwService[uuid].name[valueName]
            else:
                raise KeyError('valueName')
        else:
            raise KeyError('uuid')
    ...
    def get(cls, uuid, valueName):
        ...
        if uuid in be.Context._vnwService:
            if valueName in be.Context._vnwService[uuid].name:
                return be.Context._vnwService[uuid].name[valueName]
            else:
                raise KeyError('valueName')
        else:
            raise KeyError('uuid')
