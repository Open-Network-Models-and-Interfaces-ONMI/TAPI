...
class Context_NotifsubscriptionUuidNameValuenameImpl:
    ...
    def put(cls, uuid, valueName, nameandvalue):
        ...
        if uuid in Context._notifSubscription:
            be.Context._notifSubscription[uuid].name[valueName] = nameandvalue
        else:
    ...
    def delete(cls, uuid, valueName):
        ...
        if uuid in be.Context._notifSubscription:
            if valueName in be.Context._notifSubscription[uuid].name:
                del be.Context._notifSubscription[uuid].name[valueName]
            else:
                raise KeyError('valueName')
        else:
            raise KeyError('uuid')
    ...
    def get(cls, uuid, valueName):
        ...
        if uuid in be.Context._notifSubscription:
            if valueName in be.Context._notifSubscription[uuid].name:
                return be.Context._notifSubscription[uuid].name[valueName]
            else:
                raise KeyError('valueName')
        else:
            raise KeyError('uuid')
