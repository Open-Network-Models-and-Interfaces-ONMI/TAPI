...
class Context_ConnectivityserviceUuidNameValuenameImpl:
    ...
    def put(cls, uuid, valueName, nameandvalue):
        ...
        if uuid in Context._connectivityService:
            be.Context._connectivityService[uuid].name[valueName] = nameandvalue
        else:
    ...
    def delete(cls, uuid, valueName):
        ...
        if uuid in be.Context._connectivityService:
            if valueName in be.Context._connectivityService[uuid].name:
                del be.Context._connectivityService[uuid].name[valueName]
            else:
                raise KeyError('valueName')
        else:
            raise KeyError('uuid')
    ...
    def get(cls, uuid, valueName):
        ...
        if uuid in be.Context._connectivityService:
            if valueName in be.Context._connectivityService[uuid].name:
                return be.Context._connectivityService[uuid].name[valueName]
            else:
                raise KeyError('valueName')
        else:
            raise KeyError('uuid')
