    ...
    def get(cls, uuid, node_uuid, costAlgorithm):
        ...
        if uuid in be.Context._topology:
            if node_uuid in be.Context._topology[uuid]._node:
                if costAlgorithm in be.Context._topology[uuid]._node[node_uuid]._transferCost.costCharacteristic:
                    return be.Context._topology[uuid]._node[node_uuid]._transferCost.costCharacteristic[costAlgorithm]
                else:
