...
class Context_PathcompserviceUuid_RoutingconstraintCostcharacteristicCostalgorithmImpl:
    ...
    def get(cls, uuid, costAlgorithm):
        ...
        if uuid in Context._pathCompService:
            if costAlgorithm in Context._pathCompService[uuid]._routingConstraint.costCharacteristic:
                return Context._pathCompService[uuid]._routingConstraint.costCharacteristic[costAlgorithm]
            else:
